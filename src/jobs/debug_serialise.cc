...
std::string outputSerialiserData(const Serialiser &serialiser) {
  ...
  for (std::size_t i = 0; i < length; i += 8) {
    ...
    for(std::size_t j = 0; j < 8 && i+j < length; j++) {
      ...
      if ( i+j == length-1 ) {
        for (std::size_t space = 0; space < 8 - j - 1; space++) {
          log << "   ";
        }
      }
    }
...
QueryResult DebugSerialise::execute() {
  ...
  //TODO: updated this
  std::shared_ptr<Base> tBase = std::make_shared<Base>();
  ...
  std::shared_ptr<EntityRef> tEntRef = std::make_shared<EntityRef>((EHandle_t)1234);
